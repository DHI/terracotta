...
def test_invalid_group_insertion(monkeypatch, driver_path, provider, raster_file):
    ...
    with monkeypatch.context() as m:
        ...
        # first insertion succeeded, second failed
        assert ("bar",) in datasets
        assert ("foo",) not in datasets
...
def test_multiprocess_insertion(driver_path, provider, raster_file):
    ...
    worker = functools.partial(
        insertion_worker, path=driver_path, raster_file=raster_file, provider=provider
    )
...
def test_raster_cache(driver_path, provider, raster_file, asynchronous):
    ...
    data1 = db.get_raster_tile(
        ["some", "value"], tile_size=(256, 256), asynchronous=asynchronous
    )
...
def test_nodata_consistency(
    driver_path, provider, big_raster_file_mask, big_raster_file_nodata
):
    from terracotta import drivers
