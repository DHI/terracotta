...
@click.option(
    "--resampling-method",
    type=click.Choice(list(RESAMPLING_METHODS.keys())),
    default="average",
...
def optimize_rasters(
    ...
    with contextlib.ExitStack() as outer_env:
        outer_env.enter_context(
            click_spinner.spinner(
                beep=False, disable=quiet, force=False, stream=sys.stdout
            )
        )
        ...
        if nproc > 1:
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    raise RuntimeError(
                        f"Error while optimizing file {futures[future]}"
                    ) from exc
